					      ...
					      <tbody>
						       <c:choose>
						       		<c:when test="${empty mypage.result }">
						       			<tr><span class="label label-warning">你没有任何成功订单</span></tr>
						       		</c:when>
						       		<c:otherwise>
								       ...
								       <c:forEach items="${item.orderItems }" var="it" varStatus="statu">
								          <tr>	
								        	<td><a href="javascript:;">${it.bookname }</a></td>
									          ...
									          <c:if test="${statu.count==1 }">
									          	<td ><span class="label label-success">${item.statu }</span></td>
									          </c:if>
								       	   </tr>	
								        </c:forEach>
							        </c:forEach>
			                  ...
			                  <ul>
			                    <c:if test="${mypage.totalPages>1}">
			                    	 <c:choose>
			       					  	...
			       					  	<c:otherwise>
			       					  		<li><a href="/user/myorder/1">首页</a></li>
			       					  		<li><a href="/user/myorder/${mypage.pageNum-1}">上一页</a></li>
			       					  	</c:otherwise>
							          </c:choose>
			       					  <c:choose>
			       					  	...
			       					  	<c:otherwise>
			       					  		<li><a href="/user/myorder/${mypage.pageNum+1}">下一页</a></li>
			       					  		<li><a href="/user/myorder/${mypage.totalPages}">尾页</a></li>
			       					  	</c:otherwise>
			       					  </c:choose>
			                    </c:if>
			                  </ul>
